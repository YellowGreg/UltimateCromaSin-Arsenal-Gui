...
local Window = Library:CreateLib("AdvanceTech | Arsenal | V2")

local Tab1 = Window:CreateTab("Main")
local Tab2 = Window:CreateTab("Gun")
local Tab3 = Window:CreateTab("Visual")
local Tab4 = Window:CreateTab("Local Player")
local Tab5 = Window:CreateTab("Misc")
local Tab6 = Window:CreateTab("Credit")

local Section1 = Tab1:CreatsSection("Feature: More Script Added")
...
Section1:CreateButton("Invisible Hitbox", "No See Player Hitbox", function()
loadstring(game:HttpGet("https://pastebin.com/raw/KpQhjvRQ",true))()
end)
Section1:CreateButton("Hitbox", "Not Invisible", function()
loadstring(game:HttpGet("https://pastebin.com/raw/RrTbsWa4",true))()
end)
Section1:CreateButton("Expanded Hitbox", "Similar To Hitbox", function()
...
Section1:CreateButton("Anti Headshot", "Headshot Only", function()
    --[[
...
Section1:CreateButton("Silent Aim", "Shhh", function()
...
Section1:CreateButton("Silent Aim 2", "shhhh", function()
...
Section1:CreateButton("Silent Aim 3", "Yoooo", function()
...
Section1:CreateButton("Silent Aim 4", "i know you will get caught", function()
...
Section1:CreateButton("Silent Aim 5", "bruh idk what to say", function()
...
Button:CreateButton("AimBot(Only Detects Enemies)", function() 
local Players = game:GetService("Players")
local RunService = game:GetService("RunService")
local UserInputService = game:GetService("UserInputService")
...
Section1:CreateButton("Smooth Aimbot(keyboard=press:k)", "No Joke its smooth", function()
_G.AimbotInput = "k" -- // RightClick, LeftClick, Q, etc...
...
Section1:CreateButton("TriggerBot", "Tiggerbot=Shoots Byitself",  function()
-- // Configurations \\ --
_G.Configurations = {
   -- Basic --
   Target = "Head";
   ...
   -- Advanced --
   TriggerBot = true;                                     -- Whether the triggerbot will automatically click for you.
   PredictionSpeed = 0;                                    -- For movement prediction, set to 0 to disable.
   BulletDrop = 0;                                         -- Bullet Drop speed, set to 0 to disable.

   -- Interface --
   FOVCircle = true;

   -- Key Binds --
   Method = "Hold";                                        -- "Hold" to enable only when the key is held, "Toggle" is to toggle aimbot every time the key is pressed.
   KeyBind = Enum.UserInputType.MouseButton2;                               -- The keybind for enabling/disabling the aimbot. Can be both Keyboard or Mouse.
   TriggerBind = Enum.KeyCode.Right;
}
...
local FOVCircle = Drawing.new("Circle", {
   ...
   NumSides = 25,
   Radius = _G.Configurations.FOVRadius
})
...
local function GetClosestPlayerToCursor(Distance)
   local Closest = nil
   local Position = nil
   ...
   for i, v in ipairs(Services.Players.GetPlayers(Services.Players)) do
       if v ~= LocalPlayer and v.Team ~= LocalPlayer.Team and ValidCharacter(v.Character) then
   ...
   return Closest, ShortestDistance, Position
end

local function AimAtVector3(Position)
   if not Position then return end
   local ScreenPointPosition = Camera:WorldToScreenPoint(Position)
   ScreenPointPosition = Vector2.new(ScreenPointPosition.X, ScreenPointPosition.Y)
   local MousePosition = Services.UserInputService:GetMouseLocation() - Vector2.new(0, 36)
   local CurrentSensitivity = _G.Configurations.Sensitivity * 0.01
   mousemoverel((ScreenPointPosition.X - MousePosition.X) * CurrentSensitivity, (ScreenPointPosition.Y - MousePosition.Y) * CurrentSensitivity)
   if (ScreenPointPosition - MousePosition).Magnitude <= 10 then
       return true
   end
   return false
end
...
Services.RunService.RenderStepped:Connect(function()
   if AimbotEnabled == true then
       local Target, Distance, ViewportPosition = GetClosestPlayerToCursor(_G.Configurations.FOVRadius)
       if Target then
           local Velocity = Target.Character.PrimaryPart:GetVelocityAtPosition(Target.Character.PrimaryPart.Position) * Vector3.new(1, 0, 1)
           local Magnitude = (Target.Character.PrimaryPart.Position - LocalPlayer.Character.PrimaryPart.Position).Magnitude
           local Prediction = Velocity * (_G.Configurations.PredictionSpeed / 10) * Magnitude / 100
           if Velocity.Magnitude == 0 or _G.Configurations.PredictionSpeed <= 0 then
               Prediction = Vector3.new(0, 0, 0)
           end
           local Height = Vector3.new(0, _G.Configurations.BulletDrop, 0) * Magnitude / 100
           local TargetPart = Target.Character:FindFirstChild(_G.Configurations.Target) or Target.Character.PrimaryPart
           local CanTrigger = AimAtVector3(TargetPart.Position + Prediction + Height)
           if _G.Configurations.TriggerBot and TriggerEnabled == true and CanTrigger == true and time() - LastTriggered > 0.05 then
               LastTriggered = time()
               mouse1click()
   ...
   -- Drawing --
   FOVCircle.Radius = _G.Configurations.FOVRadius
   FOVCircle.Position = Services.UserInputService:GetMouseLocation()
   FOVCircle.Visible = _G.Enabled and _G.Configurations.FOVCircle
end)
...
Services.UserInputService.InputBegan:Connect(function(Input, GameProcessed)
   ...
   if Input.KeyCode == _G.Configurations.KeyBind or Input.UserInputType == _G.Configurations.KeyBind then
       if _G.Configurations.Method == "Toggle" then
   ...
   if Input.KeyCode == _G.Configurations.TriggerBind or Input.UserInputType == _G.Configurations.TriggerBind then
       if _G.Configurations.Method == "Toggle" then
...
Section1:CreateButton("TriggerBot 2", "You know", function()
...
Section1:CretaeButton("Wallbang(Buggy & not fix)", function()
_G.Enable = true
local MT = getrawmetatable(game)
...
Section1:CreateButton("Wallbang[New]", "Bang",function()
local mt = getrawmetatable(game)
...
Section1:CreateButton("Wallbang 3", function()
...
Section1:CreateButton("Teleport to Random Player", "Similar To Kill All", function()
...
Section1:CreateToggle("Semi wallbang", function(wallbang)
_G.Enable = wallbang
local MT = getrawmetatable(game)
local OldIndex = MT.__index
setreadonly(MT, false)
...
Section1:CreateButton("Kill All", "Kill everyone", function()
...
Section1:CreateButton("Kill All 2", "You know ok", function()
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
Section2:CreateButton("9999 Ammo", "Alot of Bullets", function()
...
Section2:CreateButton("RainBow Gun", "Not Fe But its cool", function()
local c = 1 function zigzag(X)  return math.acos(math.cos(X * math.pi)) / math.pi end game:GetService("RunService").RenderStepped:Connect(function()  if game.Workspace.Camera:FindFirstChild('Arms') then   for i,v in pairs(game.Workspace.Camera.Arms:GetDescendants()) do    if v.ClassName == 'MeshPart' then      v.Color = Color3.fromHSV(zigzag(c),1,1)     c = c + .0001    end   end  end end)
end)

Section2:CreateButton("FastFire", "Everything Gun Shoots Fast", function()
...
Section2:CreateButton("Auto Crit", "More Damage=Advantage", function()
local replicationstorage = game.ReplicatedStorage
...
Section2:CreateButton("No Reload", "Just Reloading 0.1 second Faster", function()
local replicationstorage = game.ReplicatedStorage
...
Section2:CreateButton("Remove Spread(Fix alittle)", function()
local replicationstorage = game.ReplicatedStorage
...
Section2:CreateButton("Remove Recoil(Fix alittle)", "Zero Reco", function()
 local replicationstorage = game.ReplicatedStorage
...
Section2:CreateButton("Normal GunMod", "All Mod In weapon", function()
game.StarterGui:SetCore("SendNotification", {
...
Section3:CreateButton("ESP", "Shows Enemies", function()
...
Section3:CreateButton("Skeleton Esp", "Show Stickfigure lol", function()
...
Section3:CreateButton("ESP 2", "Box Esp i think", function()
    --Made by Andrheyplayz_officalyt
local gui = Instance.new("BillboardGui");
gui.Name = "";
gui.AlwaysOnTop = true;
...
Section3:CreateButton("Tracer(Fix alittle)", "Line in your Screen", function()
...
Section3:CreateButton("Tracer 2(Wont Crash)", "You know you know", function()
...
Section3:CreateButton("Fullbright", "LightMode you can say", function()
...
Section3:CreateButton("FullBright 2", "Its LightMode 2", function()
